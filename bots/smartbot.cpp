...
bool cardknowledge::cannot_be(Color color) {
    if (color_ != -1) return (color_ != color);
    for (int v = 1; v < 6; v++) {
...
bool cardknowledge::cannot_be(Rank rank) {
    if (this->rank_ != -1) return (this->rank_ != rank);
    for (int k = 1; k < 6; k++) {
...
void cardknowledge::befuddle_by_discard() {
    ...
    if (worthless_ != YES) { valuable_ = worthless_ = MAYBE; prob_valuable_ = prob_worthless_ = -1.0f; }
}
...
void cardknowledge::befuddle_by_play(bool success) {
    ...
    if (worthless_ != YES) { worthless_ = MAYBE; prob_worthless_ = -1.0f; }
}
...
void cardknowledge::set_is_playable(bool playable, State s) {
    ...
    if (valuable_ == MAYBE) prob_valuable_ = -1.0;
    if (worthless_ == MAYBE) prob_worthless_ = -1.0;
    if (playable) { worthless_ = NO; prob_worthless_ = 0.0; }
...
void cardknowledge::set_is_valuable(bool valuable, State s) {
    ...
    if (worthless_ == MAYBE) prob_worthless_ = -1.0;
    if (valuable) { worthless_ = NO; prob_worthless_ = 0.0; }
}
...
void cardknowledge::set_is_worthless(bool worthless, State s) {
    ...
    if (worthless_ == MAYBE) prob_worthless_ = -1.0;
    if (worthless) { playable_ = valuable_ = NO; prob_playable_ = prob_valuable_ = 0.0; }
}

void cardknowledge::compute_identity() {
    if (color_ != -2 && rank_ != -2) return;
    int color = -2;
    int rank = -2;
    for (int k = 1; k < 6; k++) {
        for (int r = 1; r < 6; r++) {
...
{
    if (prob_worthless_ != -1.0f) return;
    int total_count = 0;
    int yes_count = 0;
    for (int k = 1; k < 6; k++) {
        for (int r = 1; r < 6; r++) {
...
bool cardknowledge::could_be_playable(int rank, State s) {
    if (rank < 1 || rank > 5 || cannot_be(Rank(rank))) return false;
    if (playable(s) != MAYBE) return false;
...
bool cardknowledge::could_be_valuable(int rank, State s) {
    if (rank < 1 || rank > 5 || cannot_be(Rank(rank))) return false;
    if (valuable(s) != MAYBE) return false;
    cardknowledge new_knowl = *this;
    new_knowl.set_must_be(Rank(rank));
...
bool smartbot::is_valuable(State s, Card card) {
    ...
    if (played_count_[card.color()][card.rank()] != total - 1) return false;
    return !(is_worthless(s, card));
}

bool smartbot::is_worthless(State s, Card card) {
    int v = card.rank();
    if (v < s.get_piles()[card.color()] + 1) return true;
    while (v > s.get_piles()[card.color()] + 1) {
        v--;
        int total = ((v == 1) ? 3 : ((v == 5) ? 1 : 2));
        if (played_count_[card.color()][v] == total) return true;
    }
    return false;
}
    ...
    for (int p = 0; p < hand_knowledge_.size(); p++) {
        if (p == id_) {
            for (int i = 0; i < my_hand_size_; i++) {
                if (hand_knowledge_[p][i].known()) eyesight_count_[hand_knowledge_[p][i].color()][hand_knowledge_[p][i].rank()]++;
            }
        } else {
...
int smartbot::next_discard_index(State s, int player_index) {
    ...
    for (int i = 0; i < s.get_hands()[player_index].size(); i++) {
        if (hand_knowledge_[player_index][i].playable(s) == YES) return -1;
        if (hand_knowledge_[player_index][i].worthless(s) == YES) return -1;
        if (hand_knowledge_[player_index][i].valuable(s) == YES) continue;
...
void smartbot::no_warning_given(int from, State s) {
    if (s.get_deck().size() == 0 || s.get_num_hints() == 0) return;
...
void smartbot::observe_before_discard(State s, move m) {
    ...
    if (hand_knowledge_[m.get_from()][m.get_card_index()].known() && hand_knowledge_[m.get_from()][m.get_card_index()].playable(s) == YES) {
        ...
        bool seen = false;
        for (int p = 0; p < hand_knowledge_.size(); p++) {
            if (p == m.get_from() || p == id_) continue;
...
move smartbot::discard_finesse(State s) {
    ...
    for (int i = 0; i < playable_cards.size(); i++) {
        ...
        for (int j = 0; j < partner_newest.size(); j++) {
            if (partner_newest[j] == playable_cards[i]) count++;
        }
        if (count == 1) {
...
move smartbot::play_lowest_playable(State s) {
    ...
    double best_f = 0.0;
    for (int i = 0; i < my_hand_size_; i++) {
        if (hand_knowledge_[id_][i].playable(s) == NO) continue;
        cardknowledge eye_knowl = hand_knowledge_[id_][i];
        eye_knowl.update(true);
        if (eye_knowl.playable(s) != YES) continue;
        double f = 6 - eye_knowl.rank();
        if (hand_knowledge_[id_][i].playable(s) != YES) f += 100;
        if (f > best_f) {
            best_f = f;
            best_i = i;
        }
...
move smartbot::discard_worthless(State s) {
    ...
    double best_f = 0.0;
    for (int i = 0; i < my_hand_size_; i++) {
        if (hand_knowledge_[id_][i].worthless(s) == NO) continue;

        if (hand_knowledge_[id_][i].worthless(s) == MAYBE) {
            cardknowledge eye_knowl = hand_knowledge_[id_][i];
            eye_knowl.update(true);
            if (eye_knowl.worthless(s) != YES) continue;
        }
...
std::tuple<move, int> smartbot::best_hint_for_partner_given_constraint(State s, int partner_index, F&& is_okay) {
    ...
    for (int k = 1; k < 6; k++) {
        ...
        for (int i = 0; i < partner_hand.size(); i++) {
            if (partner_hand[i].color() == Color(k)) indices.push_back(i);
        }
    ...
    for (int r = 1; r < 6; r++) {
        ...
        std::vector<int> indices = {};
        for (int i = 0; i < partner_hand.size(); i++) {
            if (partner_hand[i].rank() == Rank(r)) indices.push_back(i);
        }
...
std::tuple<move, int> smartbot::best_hint_for_partner(State s, int partner_index) {
    ...
    if (partner_index == (id_ + 1) % hand_knowledge_.size()) {
        ...
        if (discard_ind != -1) {
            ...
            if (!hand_knowledge_[partner_index][discard_ind].could_be_valuable(avoid_rank, s)) avoid_rank = invalid_rank;
        }
    }
    ...
    return best_hint_for_partner_given_constraint(s, partner_index, [&](move m, std::vector<cardknowledge> old_knowl, std::vector<cardknowledge> new_knowl) {
        if (m.get_rank() != invalid_rank && m.get_rank() == avoid_rank) return false;
        bool reveals_playable = false;
        trivalue is_misleading = MAYBE;
        for (int c = partner_hand.size() - 1; c >= 0; c--) {
            if (old_knowl[c].playable(s) != MAYBE) continue;
            bool found = false;
            for (int i = 0; i < m.get_card_indices().size(); i++) {
                if (m.get_card_indices()[i] == c) found = true;
            }
        ...
        if (reveals_playable || is_misleading == NO) return true;
        return false;
    });
}
...
move smartbot::give_valuable_warning(State s) {
    ...
    for (int i = 0; i < s.get_hands()[player_to_warn].size(); i++) {
        if (s.get_hands()[player_to_warn][i].rank() == c.rank()) indices.push_back(i);
    }
...
move smartbot::play(State s) {
    ...
    if (s.get_num_hints() == 8) {
        ...
        for (int i = 0; i < s.get_hands()[right_partner].size(); i++) {
            if (s.get_hands()[right_partner][i].rank() == s.get_hands()[right_partner][0].rank()) indices.push_back(i);
        }
